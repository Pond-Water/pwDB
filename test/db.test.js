...
todo
    ...
    */
     ...
     * in PwDB and the callback called again, transforming a user error into a PwDB error.
     *
     ...
     *
     * Note: maybe using an in-memory only PwDB would give us an easier solution
     */
  ...
  describe('Find', function () {
    ...
    it('Can find dates and objects (non JS-native types)', function (done) {
      ...
      d.insert({ now: date1, sth: { name: 'pwdb' } }, function () {
        d.findOne({ now: date1 }, function (err, doc) {
          assert.isNull(err);
          doc.sth.name.should.equal('pwdb');

          d.findOne({ now: date2 }, function (err, doc) {
            assert.isNull(err);
            assert.isNull(doc);

            d.findOne({ sth: { name: 'pwdb' } }, function (err, doc) {
              assert.isNull(err);
              doc.sth.name.should.equal('pwdb');
