...
describe('Model', function () {
  ...
  describe('Modifying documents', function () {
    ...
    describe('$pull modifier', function () {
      ...
      it('Can use any kind of pwdb query with $pull', function () {
  ...
  describe('Querying', function () {
    ...
    describe('Getting a fields value in dot notation', function () {
      ...
      it("Can navigate inside arrays with dot notation, and return the array of values in that case", function () {
        ...
        dv = model.getDotValue({ pwdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.number');
        ...
        dv = model.getDotValue({ pwdb: true, data: { planets: [ { name: 'Earth', numbers: [ 1, 3 ] }, { name: 'Mars', numbers: [ 7 ] }, { name: 'Pluton', numbers: [ 9, 5, 1 ] } ] } }, 'data.planets.numbers');
      ...
      it("Can get a single value out of an array using its index", function () {
        ...
        dv = model.getDotValue({ pwdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.2');
        assert.deepEqual(dv, { name: 'Pluton', number: 9 });
        
        // Dot notation with index in the middle
        dv = model.getDotValue({ pwdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.0.name');
...
todo
    ...
    describe('$lt', function () {
      ...
      it('Can compare strings, with or without dot notation', function () {
        model.match({ a: "pwdb" }, { a: { $lt: "nedc" } }).should.equal(true);
        model.match({ a: "pwdb" }, { a: { $lt: "neda" } }).should.equal(false);

        model.match({ a: { b: "pwdb" } }, { "a.b": { $lt: "nedc" } }).should.equal(true);
        model.match({ a: { b: "pwdb" } }, { "a.b": { $lt: "neda" } }).should.equal(false);
      });
...
*/
    ...
    describe('Array fields', function () {
      ...
      it('Field equality', function () {
        ...
        model.match({ tags: ['node', 'js', 'db'], pwdb: true }, { tags: 'js', pwdb: true }).should.equal(true);
